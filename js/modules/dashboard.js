      ...
      <tbody>
        ${recentSales.map(s => `
          <tr>
            ...
            <td><span class="chip ${s.payment === 'Crédit' ? 'chip-gold' : s.payment === 'Virement' ? 'chip-purple' : 'chip-green'}">${s.payment === 'Espèces' ? t('pay_cash').replace('💵 ','') : s.payment === 'Virement' ? t('pay_card').replace('💳 ','') : t('pay_credit').replace('📋 ','')}</span></td>
          </tr>
        `).join('')}
      </tbody>
    </table>`;
